...
export default function Admin() {
  usePageMeta({ title: "Admin", description: "Admin dashboard", noindex: true });
  const { user, isAdmin, isCaptionEditor, loading, nickname } = useAuth();
  const [activeTab, setActiveTab] = useState(isCaptionEditor && !isAdmin ? "captions" : "digital-orders");
  ...
  if (!isAdmin && !isCaptionEditor) return <Navigate to="/" replace />;
    ...
    <div className="min-h-screen flex flex-col bg-stone-50 dark:bg-stone-950">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-stone-600 dark:text-stone-400">{isAdmin ? "Manage the Chronicle from here, Senator." : "Caption Generator"}</p>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          {isAdmin ? (
            <TabsList className="grid w-full grid-cols-4 lg:grid-cols-12 mb-8 bg-stone-100 dark:bg-stone-800">
              ...
              <TabsTrigger value="forum" className="gap-2 data-[state=active]:bg-white dark:data-[state=active]:bg-stone-900 data-[state=active]:text-amber-600">
              ...
              <TabsTrigger value="twitter" className="gap-2 data-[state=active]:bg-white dark:data-[state=active]:bg-stone-900 data-[state=active]:text-sky-600">
                <Twitter className="h-4 w-4" />
                <span className="hidden sm:inline">Twitter</span>
              </TabsTrigger>
              <TabsTrigger value="captions" className="gap-2 data-[state=active]:bg-white dark:data-[state=active]:bg-stone-900 data-[state=active]:text-purple-600">
